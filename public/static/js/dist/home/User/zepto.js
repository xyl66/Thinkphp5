...
function browserRedirect(){
    ...
    if(bIsIpad||bIsIphoneOs||bIsMidp||bIsUc7||bIsUc||bIsAndroid||bIsCE||bIsWM){
        ele=$("iframe.video_iframe");
        for(var i=0;i<ele.length;i++){
            ele[i].style.cssText=ele[i].style.cssText+"width:100%;height:220px;border:0;"
        }
    }else{
        ele=$("iframe.video_iframe");
        for(var i=0;i<ele.length;i++){
            ele[i].style.cssText=ele[i].style.cssText+"width:100%;height:620px;border:0;"
        }
    }
}
...
function setCookie(name,value){  
    ...
    exp.setTime(exp.getTime() + 2592000);  //换成毫秒 
...
function tabSwitch(id) {
    var obj = null,
    oSwitch = $('#' + id),
    startIndex = oSwitch.data('index'),
    oTagUl = oSwitch.find('.relate_item'),
    oTagLi = oSwitch.children().eq(0).find('li');
    if ( !! startIndex) {
        startIndex = parseInt(startIndex) - 1;
        if (oTagLi.length > 1) {
            oTagLi.removeClass('active');
            oTagLi.eq(startIndex).addClass('active');
        }
    } else {
        startIndex = 0;
    }
    oTagUl.addClass('hide');
    oTagUl.eq(startIndex).removeClass('hide');
    $(document).on('click tap', oTagLi,
        function() {
            if ($(this).hasClass('active')) {
                return;
            }
            var i = oTagLi.index(this);
            oTagLi.removeClass('active');
            oTagLi.eq(i).addClass('active');
            oTagUl.addClass('hide');
            oTagUl.eq(i).removeClass('hide');
        });
}
function tabSwitchInit() {
    var tabObj = $('.j_tabSwitch'),
    len = tabObj.length;
    if (len == 0) {
        return false;
    }
...
$(function(){
    ...
    $("video").each(function(){
        ...
        if(vid.length<=0){
            ...
            if(!v_src){
                v_src=$(this).attr("data-id")
            }
            vid=v_src.match(/[0-9a-z]{11}(?=\.)/ig);
            $(this).attr("id",vid)
        }
        var setTimeId=setTimeout(function(){
            var src_tag=document.createElement("script");
            src_tag.src="http://vv.video.qq.com/getinfo?vids="+vid+"&platform=101001&charge=0&otype=json&defaultfmt=msd&sb=1&sdtfrom=v6010&_qv_rnd=UI1%2FUzLL6zCoWBXNA11419932151sAMgDw%3D%3D&_rnd=1419932151807";
            src_tag.onload=function(){
                if(QZOutputJson.vl.vi[0].fvkey.length<=0){
                    alert("vedio address error")
                }else{
                    document.getElementById(vid).src=QZOutputJson.vl.vi[0].ul.ui[0].url+QZOutputJson.vl.vi[0].fn+"?vkey="+QZOutputJson.vl.vi[0].fvkey
                }
            };
            
            document.getElementsByTagName("head")[0].appendChild(src_tag)
        },3000)
    });
    setTimeout(function(){        
        $(window).bind('scroll', function() {
            $("img[data-original]").each(function() {
                if(this.src != $(this).attr('data-original'))
        ...
        $("img[data-original]").each(function(){
            ...
            var btm = $(window).height()+$(window).scrollTop();
            var eltop = $(this).offset().top;
            if(eltop >= top && eltop <= btm){
                if(this.src != $(this).attr('data-original'))
                    this.src = $(this).attr('data-original');
            }
        });
    ...
    $("#focusBtn").click(function(){//分享图标
        var img = $("#jsfocus img");
        if(img.attr("src") != img.attr("_src")){
