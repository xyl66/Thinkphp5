$(function() {
            ...
            btnOK: {
                onBtnClick: function(c) {
                    ...
                    $.ajax({
                        ...
                        success: function(a) {
                            1 == a.status ? b.parents("tr").fadeOut(300,
                            function() {
                                HYD.hint("success", "恭喜您，删除成功！")
                            }) : HYD.hint("danger", "对不起，删除失败：" + a.msg),
                            $.jBox.hideloading(),
                            setTimeout(function() {
                                window.location.reload()
                            },
    ...
    $(".J_default").click(function() {
        var a = $(this).data("id");
        $.ajax({
            url: "/System/setDefaultewmDelivery",
            type: "post",
            dataType: "json",
            data: {
                id: a
            ...
            success: function(a) {
                1 == a.status ? (HYD.hint("success", "恭喜您，设置成功！"), setTimeout(function() {
                    window.location.reload()
                },
                1e3)) : HYD.hint("danger", "对不起，设置失败：" + a.msg),
                $.jBox.hideloading()
            }
        })
    }),
    $(".J_cancel_default").click(function() {
        var a = $(this).data("id");
        $.ajax({
            url: "/System/cancelDefaultewmDelivery",
            type: "post",
            dataType: "json",
            data: {
                id: a
